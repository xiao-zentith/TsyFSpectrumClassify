...
import numpy as np
import json
from src.utils.metrics.cosine_similarity import cosine_similarity
from src.regression.training.customdataset import CustomDataset
...
if __name__ == "__main__":
    ...
    # 示例：使用PathManager获取正确路径
    model = DualSimpleCNN(True, in_channels=1, out_channels=1, branch_number=4).to("cuda")
    
    # 示例模型路径（需要根据实际情况调整）
    best_model_path = pm.get_path('models', 'regression') + "/example_model.pth"
    
    # 示例数据集信息路径
    dataset_info_path = pm.get_path('configs', 'regression') + "/regression_dataset_info_Fish.json"
    
    # 示例输出文件夹
    fold_output_folder = pm.get_path('results', 'regression') + "/test_output"
    
    # 注意：以下代码仅为示例，实际使用时需要确保文件存在
