...
def process_label_folder(input_label_folder, output_label_folder):
    """
    处理单个标签文件夹中的数据
    :param input_label_folder: 输入标签文件夹路径
    :param output_label_folder: 输出标签文件夹路径
    ...
    # 复制原始文件
    copy_original_files(input_label_folder, output_label_folder)
    ...
    for i in range(len(file_paths)):
        for j in range(i + 1, len(file_paths)):
            if (i, j) not in processed_pairs and (j, i) not in processed_pairs:
                ...
                # 读取两个矩阵数据
                x_coords_a, y_coords_a, matrix_a = read_matrix_from_file(file_path_a)
                x_coords_b, y_coords_b, matrix_b = read_matrix_from_file(file_path_b)
                ...
                # 进行mixup处理
                mixed_matrix, lam = mixup_matrices(matrix_a, matrix_b)
                ...
                # 保存混合矩阵到文件
                save_mixed_matrix_to_file(x_coords_a, y_coords_a, mixed_matrix, output_file_path)
